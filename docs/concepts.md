...
Nodes are auto-created when referenced in an edge. If you create an edge referencing `"aspirin"` and no node with that ID exists, Hypabase creates it with `type="unknown"`. See [Getting Started](getting-started.md#create-nodes) for usage.
...
See [Getting Started](getting-started.md#create-a-hyperedge) for usage.
...
See the [Provenance guide](guides/provenance.md) for context managers, overrides, and querying.
