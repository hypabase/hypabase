...
1. **Hypergraph expansion** — take those entity IDs and query Hypabase for their neighbors, shared edges, and paths
1. **Provenance filtering** — use `source` and `min_confidence` to keep only trusted relationships
1. **Combine** — merge both result sets into the LLM prompt
...
See the [Hybrid Vector Pattern example](https://docs.hypabase.app/latest/examples/hybrid-vector/index.md) for a complete worked implementation with code.
